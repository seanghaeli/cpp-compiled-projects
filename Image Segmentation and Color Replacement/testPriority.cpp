...
TEST_CASE("PriorityNeighbours::insertion_removal_isempty", "[weight=1][part=priority]") {
  ...
  while (!pn.IsEmpty()) {
    PixelPoint removed = pn.Remove();
    result.push_back(removed);
    // cout<< to_string(removed.x) + " " + to_string(removed.y) + " " +to_string(removed.color.h) + " " + to_string(removed.color.s) + " " + to_string(removed.color.l)<< endl;
  }
